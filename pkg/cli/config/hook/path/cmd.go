...
import (
	...
	"github.com/ylallemant/githook-companion/pkg/cli/config/hook/path/options"
...
var rootCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if options.Current.Child {
			...
			fmt.Fprintln(cmd.OutOrStdout(), filepath.Join(configDirectory, api.GithooksDirectory))
			return nil
		}

		installationDirectory := config.GithooksPathFromConfig(configContext.Config())
